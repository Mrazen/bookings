...
func TestForm_Valid(t *testing.T) {
	...
	isValid := form.Valid()
	if !isValid {
		t.Error("got invalid when should been valid")
	}
}
...
func TestFform_IsEmail(t *testing.T) {
	postedValues := url.Values{}
	form := New(postedValues)

	form.IsEmail("x")
	if form.Valid() {
		t.Error("form shows valid email non-existing field")
	}
	postedValues = url.Values{}
	postedValues.Add("email", "lel@here.com")
	form = New(postedValues)
